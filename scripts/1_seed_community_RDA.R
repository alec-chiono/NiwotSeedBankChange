...
### Index sets
idx <- list(
  "1989 vs. 2023\n(xeric)" =list(which(sc_meta$year=="1989" & sc_meta$habitat=="xeric"),
                                   which(sc_meta$year=="2023" & sc_meta$habitat=="xeric")),
  "1989 vs. 2023\n(mesic)" =list(which(sc_meta$year=="1989" & sc_meta$habitat=="mesic"),
                                   which(sc_meta$year=="2023" & sc_meta$habitat=="mesic")),
  "Mesic vs. xeric\n(1989)"=list(which(sc_meta$year=="1989" & sc_meta$habitat=="mesic"),
                                   which(sc_meta$year=="1989" & sc_meta$habitat=="xeric")),
  "Mesic vs. xeric\n(2023)"=list(which(sc_meta$year=="2023" & sc_meta$habitat=="mesic"),
                                   which(sc_meta$year=="2023" & sc_meta$habitat=="xeric"))
)
...
# Build plot data
plot_data <- imap_dfr(idx, ~ tibble(
  comparison=.y,
  `Species replacement`=mean(bal_mat[.x[[1]], .x[[2]]]),
  `Abundance gradient` =mean(gra_mat[.x[[1]], .x[[2]]])
)) %>%
  pivot_longer(-comparison, names_to="component", values_to="dissimilarity") %>%
  mutate(
    component   =factor(component, levels=c("Species replacement", "Abundance gradient")),
    comparison  =factor(comparison, levels=names(idx)),
    comparison_type=ifelse(grepl("vs\\. 2023|vs\\. 1989", comparison), "Year", "Habitat")
  )

# Plot
figS1 <- ggplot(plot_data, aes(x=comparison, y=dissimilarity, fill=component)) +
  geom_col(width=0.6, color="white", linewidth=0.3) +
  scale_fill_manual(values=c("Species replacement"="gray30", "Abundance gradient"="gray75"), name=NULL) +
...
indval_formatted <- indval_result$sign %>%
  ...
  mutate(
    ...
    group = case_when(
      `s.1989_mesic` == 1 & `s.2023_mesic` == 0 & `s.1989_xeric` == 0 & `s.2023_xeric` == 0 ~ "1989 — mesic only",
      `s.1989_xeric` == 1 & `s.2023_xeric` == 0 & `s.1989_mesic` == 0 & `s.2023_mesic` == 0 ~ "1989 — xeric only",
      `s.1989_mesic` == 1 & `s.1989_xeric` == 1 & `s.2023_mesic` == 0 & `s.2023_xeric` == 0 ~ "1989 — both habitats",
      `s.2023_mesic` == 1 & `s.2023_xeric` == 0 & `s.1989_mesic` == 0 & `s.1989_xeric` == 0 ~ "2023 — mesic only",
      `s.2023_xeric` == 1 & `s.2023_mesic` == 0 & `s.1989_mesic` == 0 & `s.1989_xeric` == 0 ~ "2023 — xeric only",
      `s.2023_mesic` == 1 & `s.2023_xeric` == 1 & `s.1989_mesic` == 0 & `s.1989_xeric` == 0 ~ "2023 — both habitats",
...
ft <- flextable(indval_formatted) %>%
  ...
  # Add caption
  set_caption("Table S3. Indicator species analysis results. Species significantly associated
