...
ft_s1 <- flextable(seedbank_wide) %>%
  ...
  add_header_row(
    values  = c("",
               ...
               "1989", "2023"),
    colwidths = c(1,
                  sum(grepl("mesic", names(seedbank_wide)[-1]) & grepl("1989", names(seedbank_wide)[-1])),
                  sum(grepl("mesic", names(seedbank_wide)[-1]) & grepl("2023", names(seedbank_wide)[-1])),
                  sum(grepl("xeric", names(seedbank_wide)[-1]) & grepl("1989", names(seedbank_wide)[-1])),
                  sum(grepl("xeric", names(seedbank_wide)[-1]) & grepl("2023", names(seedbank_wide)[-1])))
  ) %>%
  ...
  bold(part = "header") %>%
  set_caption("Table S1. Mean \u00b1 SE of seed density per four 221 cm\u00b3 cores") %>%
  theme_booktabs() %>%
  autofit() %>% fit_to_width(max_width=8)
...
ft_s2 <- flextable(veg_wide) %>%
  ...
  align(j = "USDA_name", align = "left", part = "all") %>%
  bold(part = "header") %>%
  set_caption("Table S2. Mean \u00b1 SE of vegetation hits per 1 m\u00b2 plot in 2024.") %>%
  theme_booktabs() %>%
  autofit()
