...
librarian::shelf(tidyverse, cmdstanr, tidybayes, bayesplot, posterior, ggdist, patchwork)
...
ppc4A <- ppc_stat_grouped(y_seed, seed_rep, group=USDA_lookup$USDA_code[dlist$seed_species], stat="mean")
ppc4B <- ppc_stat_grouped(y_seed, seed_rep, group=USDA_lookup$USDA_code[dlist$seed_species], stat ="sd")
ppc4C <- ppc_stat_grouped(y_veg,  veg_rep,  group=USDA_lookup$USDA_code[dlist$veg_species], stat="mean")
ppc4D <- ppc_stat_grouped(y_veg, veg_rep, group=USDA_lookup$USDA_code[dlist$veg_species], stat ="sd")

ppc4 <- (ppc4A + ppc4B) / (ppc4C + ppc4D) + plot_annotation(tag_level="A")
ggsave("figures/figS5.pdf", ppc4, width=10, height=10, units="in", dpi=600)
...
fig3A <- seed_draws %>%
  ...
  scale_x_continuous(name="Scaled Change per Year in Seed Bank", limits=c(-0.2,0.2)) +
  scale_y_discrete(name="Species") +
  scale_fill_manual(values=c("mesic"="green4", "xeric"="tan4"), name="Habitat") +
...
# MODEL w/o HABITAT ------------------------------------------------------------
## Compile
mod4_wo_habitat <- cmdstan_model("scripts/stan/compare_seed_veg_change_wo_habitat.stan")
...
## Posterior Predictive Checks
### Extract draws
seed_rep_wo_habitat <- fit4_wo_habitat$draws("seed_change_rep", format="matrix")
veg_rep_wo_habitat  <- fit4_wo_habitat$draws("veg_count_rep",   format="matrix")
y_seed   <- dlist$seed_change
y_veg    <- dlist$veg_count

### Grouped by habitat and species
ppc_stat_grouped(y_seed, seed_rep_wo_habitat, group=USDA_lookup$USDA_code[dlist$seed_species], stat="mean")
ppc_stat_grouped(y_seed, seed_rep_wo_habitat, group=USDA_lookup$USDA_code[dlist$seed_species], stat ="sd")
ppc_stat_grouped(y_veg,  veg_rep_wo_habitat,  group=USDA_lookup$USDA_code[dlist$veg_species], stat="mean")
ppc_stat_grouped(y_veg, veg_rep_wo_habitat, group=USDA_lookup$USDA_code[dlist$veg_species], stat ="sd")
...
seed_draws_wo_habitat <- lapply(USDA_lookup$USDA_code_id, function(x) {
  ...
  do.call(rbind, .) %>%
  mutate(USDA_name = factor(USDA_lookup$USDA_name[USDA_code_id], levels = spp_order))
...
figS3 <- (figS3A | figS3B + plot_layout(axes="collect")) / figS3C +
  plot_layout(guides="collect") + plot_annotation(tag_level="A")
